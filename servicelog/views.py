...
def servicelog_overview(request, machine_id = None):
    ...
    if machine_id:
        ...
        entries = entries.order_by('machine', '-last_updated').distinct('machine').order_by('machine')
...
def servicelog_crud(request, machine_id = None, servicelog_id = None):
    try:
       machine = Machine.objects.get(id=machine_id)
       most_recent = Servicelog.objects.all().filter(machine_id = machine_id).order_by('-reported').first()
    except ObjectDoesNotExist as e:
       return HttpResponse("Machine not found",status=404,content_type="text/plain")
    ...
    if request.POST:
     form = ServicelogForm(request.POST or None, request.FILES, instance = servicelog, canreturntoservice = machine.canInstruct(request.user))
     if form.is_valid():
        try:
            item = form.save(commit = False)
            item.machine = machine
            item.reporter = request.user
            item.changeReason = "Changed by {} via service log issue reporting".format(request.user)
            item.save()

            old_state = machine.out_of_order
            new_state = form.cleaned_data.get('out_of_order')
            if old_state != new_state and (most_recent == None or item.id == most_recent.id or servicelog_id == None):
                machine.out_of_order = new_state
                machine.changeReason = "Changed by {} via servicelog issue reporting".format(request.user)
                machine.save()
    ...
    form = ServicelogForm(instance = servicelog,  canreturntoservice = machine.canInstruct(request.user))
    if servicelog != None and most_recent != None and servicelog != most_recent:
          form.fields['out_of_order'].disabled = True
          form.fields['out_of_order'].help_text = "Cannot change this - as there is a more recent servicelog entry in force."

    if servicelog:
          form.fields['out_of_order'].value = machine.out_of_order
          form.fields['out_of_order'].help_text = form.fields['out_of_order'].help_text + "<p>Shown is the <i>current</i> state of the machine."
