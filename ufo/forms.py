...
class UfoZipUploadForm(forms.Form):
    ...
    description = forms.CharField(
        max_length=300, required=False, help_text="Will be auto filled if left empty"
    )
    deadline = forms.DateField(
        required=False,
        help_text="Will be auto calcuated if left empty (%d days from now)"
        % settings.UFO_DEADLINE_DAYS,
    )
    dispose_by_date = forms.DateField(
        required=False,
        help_text="Will be auto calcuated if left empty (%d after the above deadline)"
