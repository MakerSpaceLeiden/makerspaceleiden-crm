...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
         ...
         parser.add_argument(
            '--to',
            dest='to', type = str,
            help='Sent the message to a different addres (default is ' + settings.MAILINGLIST + ').',
         )
    ...
    def handle(self, *args, **options):
        ...
        postDisposeline = Ufo.objects.filter((Q(state = 'UNK') & Q(dispose_by_date__lt=deadline)) |  Q(state = 'DEL'))
        ...
        if items:
           emailUfoInfo(list(items.values()), 'email_notification_reminder', dests, context )
