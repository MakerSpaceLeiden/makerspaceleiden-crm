...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--days',
            dest='days', action='store', default = 100, type=int,
    ...
    def handle(self, *args, **options):
        ...
        if  not options['all']:
            balances = balances.filter(Q(balance__gt = Money(0,EUR)) | Q(balance__lt  = Money(0,EUR))).filter(Q(last__date__gt = cutoff_date))
