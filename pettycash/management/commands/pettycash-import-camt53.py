...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
     ...
     Amount:       %s
     Omschrijving: %s
''' % (out.get('iban_str',None),out.get('name_str',None),out.get('amount_str',None),out.get('details',None)))
           ...
           try:
               tx = PettycashTransaction(src=User.objects.get(id = settings.POT_ID), dst=out['user'], amount=out['amount'])
               tx.description = out['description']
               tx._change_reason = "TXREF=%s HOLDER=%s IBAN=%s by script, ran by %s" % (out['ref'], out['name_str'],out['iban_str'], pwd.getpwuid( os.getuid() ).pw_name)
               if not dryrun:
                   tx.save()
               if verbosity < 2:
                   print('Txref: %s' % (out.get('ref','???')))
