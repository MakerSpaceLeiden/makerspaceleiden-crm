...
def import_pricelist(verbosity, dryrun, files):
    rc = 0
    for file in files:
        if verbosity > 1:
        ...
        for line in file:
            ...
            if wasCreated:
                sku.changeReason = "Added during bulk import from file %s" % file.name
            else:
                sku.changeReason = "Updated during bulk import from file %s" % file.name
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        rc = import_pricelist(
            int(options["verbosity"]), options["dryrun"], options["pricelist-csv-file"]
        )
        sys.exit(rc)
