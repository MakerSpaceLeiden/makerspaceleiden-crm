...
def camt53_process(file):
        print(file)
        xmlparser = etree.XMLParser(ns_clean=True,remove_blank_text=True,remove_comments=True,no_network=True)
        ...
        for e in triodos.xpath('/camt:Document/camt:BkToCstmrStmt/camt:Stmt/camt:Ntry',namespaces=namespaces):
...
def process(e,namespaces):
        ...
        name_str=  next(item for item in [
           e.xpath('camt:NtryDtls/camt:TxDtls/camt:RltdPties/camt:Cdtr/camt:Nm/text()',namespaces=namespaces) +
           e.xpath('camt:NtryDtls/camt:TxDtls/camt:RltdPties/camt:Dbtr/camt:Nm/text()',namespaces=namespaces)
        ] if item is not None and item != '')[0]
    
        iban_str =  next(item for item in [
        	e.xpath('camt:NtryDtls/camt:TxDtls/camt:RltdPties/camt:DbtrAcct/camt:Id/camt:IBAN//text()',namespaces=namespaces) +
        	e.xpath('camt:NtryDtls/camt:TxDtls/camt:RltdPties/camt:CdtrAcct/camt:Id/camt:IBAN//text()',namespaces=namespaces)
        ]  if item is not None and item != '')[0]
        iban_str = '%s*****%s' % (iban_str[0:8], iban_str[-3:])
    
        details = e.xpath('camt:NtryDtls/camt:TxDtls/camt:AddtlTxInf/text()',namespaces=namespaces)
        if len(details):
           details = details[0]
        else:
           details = ''
        ...
        if amount < Money(0,EUR) or amount > settings.MAX_PAY_DEPOSITI:
            out['msg'] = "ERROR Skipping - amount not between 0 and %s" % settings.MAX_PAY_DEPOSITI
            out['error'] = True
            return out
    
        matches = [w for w in ['space', 'tegoed', 'zwarte pot', 'zwartepot', 'storting'] if w in details.lower()]
        if len(matches)<2:
        ...
        out['msg'] =("PROCESSED -- deposit %s for user %s from %s" %(amount, user, iban_str))
