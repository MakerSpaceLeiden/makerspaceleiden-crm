...
def index(request):
  ...
  return render(request, 'sensors.html', { 
               ...
               'items': items,
		'user': request.user,
                'canedit' : request.user.is_privileged or request.user.groups.filter(name=settings.SENSOR_USER_GROUP).exists()
         })
...
def create(request):
    if request.method == "POST":
      ...
      if form.is_valid():
       try:
           form.changeReason = 'Sensor Created through the self-service interface by {0}'.format(request.user)
           form.save()

           return redirect('mainsindex')
       except Exception as e:
         logger.error("Unexpected error during create of new mainssensor : {0}".format(e))
    else:
...
def modify(request,pk):
    ...
    if request.method == "POST":
     form = MainssensorForm(request.POST or None, request.FILES, instance = mainssensor)
     if form.is_valid():
       logger.error("saving")
       try:
           mainssensor.changeReason = 'Updated through the self-service interface by {0}'.format(request.user)
           mainssensor.save()
         ...
         logger.error("Unexpected error during save of mainssensor: {0}".format(e))
    else:
