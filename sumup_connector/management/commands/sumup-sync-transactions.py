        ...
        for tx in txs:
            ...
            if tx["transaction_id"] != c.transaction_id:
                if c.transaction_id is None:
                    print("Missing transaction_id (known sumup bug)")
                    c.transaction_id = tx["transaction_id"]
                    if options["update"]:
                        c = c.save()
                else:
                    print(
                        f"Transaction f{tx['transaction_id']}, mismatching with {c.pk}: {c.transaction_id}"
                    )

            if tx["status"] == "SUCCESSFUL" and c.state != "SUCCESSFUL":
                print(
                    f"Transaction f{tx['transaction_id']}, #{c.pk}: crm:{c.state} != sumup:{tx['status']} -- missing deposit"
                )
            elif tx["status"] != "SUCCESSFUL" and c.state == "SUCCESSFUL":
                print(
                    f"Transaction f{tx['transaction_id']}, #{c.pk}: crm:{c.state} != sumup:{tx['status']} -- deposit needs to be rolled back"
                )
            elif tx["status"] == "FAILED" and c.state == "CANCELLED":
                pass
            elif tx["status"] != c.state:
                print(
                    f"Transaction f{tx['transaction_id']}, #{c.pk}: crm:{c.state} != sumup:{tx['status']}"
                )

        for c in Checkout.objects.filter(state="SUCCESSFUL"):
            if c.transaction_id is not None and c.transaction_id not in tids:
                print(
                    f"#{c.pk}: transaction id {c.transaction_id} not at sumup; but paid out"
                )
            if (
                c.client_transaction_id is not None
                and c.client_transaction_id not in ctids
            ):
                print(
                    f"#{c.pk}: client transaction id {c.client_transaction_id} not at sumup; but paid out"
                )
