...
class SearchAutoCompleteAdmin(admin.ModelAdmin):
    ...
    def search_api(self, request: HttpRequest, search_term: str) -> HttpResponse:
        ...
        if len(self.search_fields) == 0:
            return HttpResponseBadRequest(
                reason="Mo search_fields defined in {}".format(self.__class__.__name__)
            )

        query = [
            Q(**{"{}{}".format(field, self.search_prefix): search_term})
        ...
        return JsonResponse(
            data=[
                {
                    "keyword": self.get_instance_name(item),
                    "url": self.get_instance_url(item),
                }
                for item in self.model.objects.filter(query)[0 : self.max_results]
            ],
            safe=False,
        )
    ...
    def get_instance_url(self, instance: Model) -> str:
        ...
        url_name = "admin:{}_{}_change".format(
            self.model._meta.app_label, str(self.model.__name__).lower()
        )
