...
def AgendaItemsView(request, pk=None):
    ...
    if show_history == 'off':
        agenda_list = Agenda.objects.filter(
            Q(enddate__gte=date.today())
        ).order_by('enddate', 'endtime', 'startdate', 'starttime')  # Filter out items with endtime and date after now
    else:
        agenda_list = Agenda.objects.all().order_by('enddate', 'endtime','startdate', 'starttime' )
    ...
    if agenda_list.exists():
        if pk:
            ...
            selected_item = Agenda.objects.filter(
